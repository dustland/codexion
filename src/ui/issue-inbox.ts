...
export const INSTALL_ISSUE_INBOX_EXPRESSION = `(() => {
  ...
  const renderNativeSettings = () => {
    ...
    const ageSelect=nativePage.querySelector("[data-codexion-age]");ageSelect.value=settingsSnapshot.maxAgeDays===null?"all":String(settingsSnapshot.maxAgeDays);
  ...
  const openNativePage = () => {
    ...
    nativePage.innerHTML=\`<style>
      ...
      #codexion-settings-page .cx-filter{background:var(--color-token-bg-primary);border:1px solid var(--color-token-border);border-radius:8px;box-sizing:border-box;color:inherit;font-size:13px;height:36px;margin-bottom:10px;outline:none;padding:0 11px;width:100%} #codexion-settings-page .cx-select{align-items:center;background:var(--color-token-bg-primary);border:1px solid var(--color-token-border);border-radius:8px;box-sizing:border-box;display:flex;flex:none;height:34px;position:relative;width:148px} #codexion-settings-page .cx-select:focus-within{border-color:color-mix(in srgb,currentColor 45%,transparent);box-shadow:0 0 0 1px color-mix(in srgb,currentColor 16%,transparent)} #codexion-settings-page .cx-age{-webkit-appearance:none;appearance:none;background:transparent;border:0;color:inherit;cursor:pointer;font-size:13px;height:100%;outline:none;padding:0 34px 0 12px;width:100%} #codexion-settings-page .cx-select svg{color:var(--color-token-text-tertiary);height:14px;pointer-events:none;position:absolute;right:10px;width:14px}
    ...
    </style><div class="cx-wrap"><h1>Codexion</h1><div class="cx-section-head"><h2>GitHub Issue Inbox</h2><span class="cx-account" data-codexion-account hidden></span></div><p>Select the repositories Codexion should poll. Issues you authored or already replied to are excluded. Handling an issue starts one idempotent Codex task in the matching local workspace.</p><div class="cx-option"><div class="cx-option-copy"><span class="cx-option-title">Issue window</span><span class="cx-option-description">Controls the recent issues shown in the Inbox and its badge.</span></div><div class="cx-select"><select class="cx-age" data-codexion-age aria-label="Issue window"><option value="3">Last 3 days</option><option value="7">Last 7 days</option><option value="14">Last 14 days</option><option value="30">Last 30 days</option><option value="all">All open issues</option></select><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="m4 6 4 4 4-4" stroke-linecap="round" stroke-linejoin="round"/></svg></div></div><input class="cx-filter" data-codexion-filter placeholder="Filter repositories"/><div class="cx-card"><div class="cx-status" data-codexion-status>Checking GitHub CLI…</div><div class="cx-repos" data-codexion-repos></div></div><section class="cx-ignored-section"><h2>Ignored Issues</h2><p>Issues hidden from the Inbox on this Mac.</p><div class="cx-card"><div class="cx-ignored-empty" data-codexion-ignored-empty>No ignored issues.</div><div data-codexion-ignored></div></div></section></div>\`;
    main.append(nativePage);
    nativePage.querySelector("[data-codexion-filter]").oninput=()=>renderNativeSettings();
    nativePage.querySelector("[data-codexion-age]").onchange=(event)=>{const value=event.target.value;queue({type:"set-max-age",maxAgeDays:value==="all"?null:Number(value)});};
