...
export const INSTALL_ISSUE_INBOX_EXPRESSION = `(() => {
  ...
  const place = () => {
    ...
    const localGroup=anchor?.parentElement;let group=localGroup;
    for(let element=localGroup;element&&element.parentElement;element=element.parentElement){if(element.classList?.contains("ms-auto")){group=element;break;}}
