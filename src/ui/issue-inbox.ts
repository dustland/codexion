...
export const INSTALL_ISSUE_INBOX_EXPRESSION = `(() => {
  ...
  shadow.innerHTML = \`
    ...
    <button class="trigger" aria-label="Open GitHub issue inbox" aria-expanded="false">
      <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><circle cx="8" cy="8" r="6.25"/><circle cx="8" cy="8" r="1.25" fill="currentColor" stroke="none"/></svg><span class="badge"></span>
    </button>
  ...
  shadow.querySelector(".header-title svg")?.replaceWith(trigger.querySelector("svg").cloneNode(true));
  ...
  const openNativePage = () => {
    ...
    nativePage.innerHTML=\`<style>
      ...
      #codexion-settings-page .cx-access,#codexion-settings-page .cx-maintenance{margin-top:36px} #codexion-settings-page .cx-settings-row{align-items:center;display:flex;gap:24px;justify-content:space-between;min-height:64px;padding:0 16px} #codexion-settings-page .cx-settings-copy{display:flex;flex:1;flex-direction:column;gap:3px;min-width:0} #codexion-settings-page .cx-settings-title{font-size:13px;font-weight:600} #codexion-settings-page .cx-settings-detail,#codexion-settings-page .cx-version{color:var(--color-token-text-secondary);font-size:11px;line-height:16px} #codexion-settings-page .cx-button-group{align-items:center;display:flex;flex:none;gap:8px} #codexion-settings-page .cx-restart-status,#codexion-settings-page .cx-rescan-status{border-top:1px solid var(--color-token-border);color:var(--color-token-text-secondary);font-size:11px;padding:9px 16px}
    </style><div class="cx-wrap"><h1>Codexion</h1><div class="cx-section-head"><h2>GitHub Issue Inbox</h2><span class="cx-account" data-codexion-account hidden></span></div><p>Select the repositories Codexion should poll. Issues you authored or already replied to are excluded. Handling an issue starts one idempotent Codex task in the matching local workspace.</p><div class="cx-option"><div class="cx-option-copy"><span class="cx-option-title">Issue window</span><span class="cx-option-description">Controls the recent issues shown in the Inbox and its badge.</span></div><div class="cx-select"><button class="cx-age" data-codexion-age aria-label="Issue window" aria-haspopup="listbox" aria-expanded="false"><span data-codexion-age-label>Last 3 days</span><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="m4 6 4 4 4-4" stroke-linecap="round" stroke-linejoin="round"/></svg></button><div class="cx-age-menu" data-codexion-age-menu role="listbox" aria-label="Issue window"><button id="codexion-age-3" class="cx-age-option" data-codexion-age-option="3" role="option">Last 3 days</button><button id="codexion-age-7" class="cx-age-option" data-codexion-age-option="7" role="option">Last 7 days</button><button id="codexion-age-14" class="cx-age-option" data-codexion-age-option="14" role="option">Last 14 days</button><button id="codexion-age-30" class="cx-age-option" data-codexion-age-option="30" role="option">Last 30 days</button><button id="codexion-age-all" class="cx-age-option" data-codexion-age-option="all" role="option">All open issues</button></div></div></div><div class="cx-card"><div class="cx-filter-wrap"><div class="cx-filter-control"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><circle cx="7" cy="7" r="4.25"/><path d="m10.25 10.25 3 3" stroke-linecap="round"/></svg><input class="cx-filter" data-codexion-filter aria-label="Filter repositories" placeholder="Filter repositories"/></div></div><div class="cx-status" data-codexion-status>Checking GitHub CLI…</div><div class="cx-repos" data-codexion-repos></div></div><section class="cx-access"><h2>File Access</h2><p>Codexion only checks workspaces already known to Codex. Full Disk Access is optional and may be needed for workspaces in protected folders.</p><div class="cx-card"><div class="cx-settings-row"><div class="cx-settings-copy"><span class="cx-settings-title">Workspace access</span><span class="cx-settings-detail">After changing access, rescan once to update local repository mappings.</span></div><div class="cx-button-group"><button class="cx-secondary" data-codexion-open-access>Open System Settings</button><button class="cx-secondary" data-codexion-rescan>Rescan</button></div></div><div class="cx-rescan-status" data-codexion-rescan-status hidden></div></div></section><section class="cx-ignored-section"><h2>Ignored Issues</h2><p>Issues hidden from the Inbox on this Mac.</p><div class="cx-card"><div class="cx-ignored-empty" data-codexion-ignored-empty>No ignored issues.</div><div data-codexion-ignored></div></div></section><section class="cx-maintenance"><h2>Maintenance</h2><p>Restart the background companion without restarting Codex Desktop.</p><div class="cx-card"><div class="cx-settings-row"><div class="cx-settings-copy"><span class="cx-settings-title">Codexion Core</span><span class="cx-version" data-codexion-version></span></div><button class="cx-secondary" data-codexion-restart>Restart Codexion</button></div><div class="cx-restart-status" data-codexion-restart-status hidden></div></div></section></div>\`;
    ...
    nativePage.querySelector("[data-codexion-open-access]").onclick=()=>queue({type:"open-full-disk-access"});
    const rescanButton=nativePage.querySelector("[data-codexion-rescan]");const rescanStatus=nativePage.querySelector("[data-codexion-rescan-status]");rescanButton.onclick=()=>{rescanButton.disabled=true;rescanButton.textContent="Scanning…";rescanStatus.hidden=false;rescanStatus.textContent="Checking known Codex workspaces…";queue({type:"rescan-workspaces"});};
    const restartButton=nativePage.querySelector("[data-codexion-restart]");const restartStatus=nativePage.querySelector("[data-codexion-restart-status]");restartButton.onclick=()=>{restartButton.disabled=true;restartButton.textContent="Restarting…";restartStatus.hidden=false;restartStatus.textContent="Restarting the background companion…";queue({type:"restart-codexion"});setTimeout(()=>{if(!restartButton.isConnected)return;restartButton.disabled=false;restartButton.textContent="Restart Codexion";restartStatus.textContent="Codexion did not restart. Check the local log and try again.";},10000);};
  ...
  window.__codexionUpdateIssueInbox = (next, isSettings=false) => { if(isSettings){settingsSnapshot=next; renderSettings(); renderNativeSettings();if(nativePage){const button=nativePage.querySelector("[data-codexion-rescan]");const status=nativePage.querySelector("[data-codexion-rescan-status]");if(button?.disabled){button.disabled=false;button.textContent="Rescan";status.hidden=false;status.textContent="Workspace scan complete.";}}} else {snapshot=next; renderIssues();} errorBox.hidden=true; };
